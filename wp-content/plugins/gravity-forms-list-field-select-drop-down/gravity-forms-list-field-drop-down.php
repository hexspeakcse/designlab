...
if ( !class_exists('ITSG_GF_List_Field_Drop_Down') ) {
    class ITSG_GF_List_Field_Drop_Down {
        ...
        */
		...
		function drop_down_maybe_string_to_array( $choices ) {
		...
		function add_user_option( $form ) {
			...
			if ( is_array( $form ) || is_object( $form ) ) {
				...
				} else if ( GFCommon::is_entry_detail() ) { // if viewing entry in entry editor
				...
				foreach ( $form['fields'] as &$field ) {  // for all form fields
					...
					if ( 'list' == $field->get_input_type() ) {
						...
						} else if ( GFCommon::is_entry_detail() ) {
							...
							foreach ( $list_values as $row ) { // get each row
								foreach ( $field->choices as $key => &$choice ) { // for each column
									...
									if ( $isDropDown ) {
										...
										$choices = $this->drop_down_maybe_string_to_array( $choices );
				...
				if ( $has_columns ) {
					foreach( $field['choices'] as $choice ) {
						if ( $column == $choice['text']  &&  rgar( $choice, 'isDropDown' ) && '' != rgar( $choice, 'isDropDownChoices' ) ) {
							...
							$isDropDownChoices = $this->drop_down_maybe_string_to_array( $isDropDownChoices );
							...
							//if ( ! empty( $value ) && '' != $value && ! array_filter( $isDropDownChoices ) ) {
							//	array_unshift( $isDropDownChoices, $value );  // push current value into select list if options list is empty
							//}
